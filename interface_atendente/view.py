...
class avelar_view():
    ...
    def gerenciamento(self):
        ...
        self.botao_excluir_sala = Button(self.tela, text = "Excluir Sala",height = "10", width = "20", bd = "10", relief = "flat")
            #Posição dos Botões
        self.botao_nova_sala.place(x = self.x[0], y = self.y)
        self.botao_editar_sala.place(x = self.x[1], y = self.y)
        self.botao_excluir_sala.place(x = self.x[2], y = self.y)
            #Funções dos Botões
    ...
    def tela_editar_sala(self, salas_existentes):
        ...
        if len(salas_existentes) is not 0:
            ...
            self.texto_salas_disponiveis = Label(self.tela, text = "Nenhuma Sala Existente", font=('Verdana','32','italic','bold'), bd = "2", relief = "flat")
        ...
        self.botao_voltar.pack(expand = True)
            #Funções dos Botões
        self.botao_voltar["command"] = lambda: self.gerenciamento()
    ...
    def popup_selecionar_e_editar_velorio(self, sala):
        ...
        #Criação da Lista de Seleção
        mostra_velorios = Listbox(self.pop_up, height = "5", width = "50", bd = "2", relief = "flat", bg = "#040c31", fg = "white", font=('Verdana','12','italic','bold'))
        mostra_velorios.bind('<<ListboxSelect>>', self.velorio_selecionado)
            #Alimentando Lista de Seleção
        for velorio in velorios:
            texto_velorio = f"{velorio[0]} - {velorio[2]} - Hora de Saída = {velorio[3]}"
    ...
    def popup_editar_velorio(self, velorio):
        ...
        nome = str(velorio[0][2])
        hora_saida = str(velorio[0][3])
        data_sepultamento = str(velorio[0][4])
        cemiterio_sepultamento = str(velorio[0][5])
        status = bool(velorio[0][6])
        #Criação da Tela pop_up
        pop_up = Tk()
        #elf.pop_up.overrideredirect(True)
        ...
        self.botao_enviar.pack(padx = "10", pady = "10")
            #Funções dos Botões
        self.botao_enviar["command"] = lambda: self.pega_dados_do_velorio([int(velorio[0][0]), self.caixa_pega_nome.get(), self.caixa_pega_horario.get(), 
